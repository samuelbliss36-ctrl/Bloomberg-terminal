...
const COL_W = { rank: 32, ticker: 72, name: 180, price: 80, chg: 72, pct: 72 };

function MoversTable({ title, data, loading, color, bgHeader }) {
  return (
    <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", border: "1px solid #334155", borderRadius: 6, overflow: "hidden", fontFamily: "'IBM Plex Mono', monospace" }}>

      {/* Sheet-style header */}
      <div style={{ background: bgHeader, padding: "8px 12px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "2px solid #334155" }}>
      ...
      <div style={{ display: "flex", background: "#1e293b", borderBottom: "1px solid #334155" }}>
        {[["#", COL_W.rank], ["Ticker", COL_W.ticker], ["Name", COL_W.name], ["Price", COL_W.price], ["Chg $", COL_W.chg], ["Chg %", COL_W.pct]].map(([label, w]) => (
          <div key={label} style={{ width: w, flexShrink: 0, padding: "5px 8px", fontSize: 9, color: "var(--text-3)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", borderRight: "1px solid #334155", textAlign: label === "#" ? "center" : label === "Price" || label === "Chg $" || label === "Chg %" ? "right" : "left" }}>
            {label}
          </div>
        ))}
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", background: "var(--surface-0)" }}>
        {loading ? (
          Array.from({ length: 10 }).map((_, i) => (
            <div key={i} style={{ display: "flex", borderBottom: "1px solid #1e293b", padding: "6px 0", background: i % 2 === 0 ? "var(--surface-0)" : "var(--surface-1)" }}>
              {[COL_W.rank, COL_W.ticker, COL_W.name, COL_W.price, COL_W.chg, COL_W.pct].map((w, j) => (
          ...
          data.map((row, i) => (
            <div key={row.symbol} style={{
              ...
              background: i % 2 === 0 ? "var(--surface-0)" : "rgba(30,41,59,0.5)",
              transition: "background 0.1s",
            }}
              onMouseEnter={e => e.currentTarget.style.background = "rgba(37,99,235,0.08)"}
              onMouseLeave={e => e.currentTarget.style.background = i % 2 === 0 ? "var(--surface-0)" : "rgba(30,41,59,0.5)"}
            >
              {/* Rank */}
              <div style={{ width: COL_W.rank, flexShrink: 0, padding: "7px 8px", textAlign: "center", fontSize: 10, color: "var(--text-3)", borderRight: "1px solid #1e293b" }}>
                {i + 1}
              </div>
              {/* Ticker */}
              <div style={{ width: COL_W.ticker, flexShrink: 0, padding: "7px 8px", fontSize: 11, fontWeight: 700, color, borderRight: "1px solid #1e293b" }}>
                {row.symbol}
              </div>
              {/* Name */}
              <div style={{ width: COL_W.name, flexShrink: 0, padding: "7px 8px", fontSize: 10, color: "var(--text-2,#cbd5e1)", borderRight: "1px solid #1e293b", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {row.name}
              </div>
              {/* Price */}
              <div style={{ width: COL_W.price, flexShrink: 0, padding: "7px 8px", fontSize: 11, color: "var(--text-1)", textAlign: "right", borderRight: "1px solid #1e293b" }}>
                ${row.price?.toFixed(2)}
              </div>
              {/* Chg $ */}
              <div style={{ width: COL_W.chg, flexShrink: 0, padding: "7px 8px", fontSize: 11, color, textAlign: "right", borderRight: "1px solid #1e293b", fontWeight: 600 }}>
...
export default function DayMovers({ onContextUpdate }) {
  ...
  useEffect(() => {
    fetchMovers();
    const id = setInterval(fetchMovers, 5 * 60_000); // refresh every 5 min
