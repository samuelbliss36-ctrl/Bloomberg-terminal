...
function OilPrices() {
  const [data, setData] = useState({ wti: null, brent: null });
  ...
  useEffect(() => {
    ...
    Promise.all([fetchQuote("CL=F"), fetchQuote("BZ=F")])
      .then(([wti, brent]) => setData({ wti, brent }))
      .catch(() => {});
  }, []);
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        {[["WTI", "CL=F", data.wti], ["Brent", "BZ=F", data.brent]].map(([label, ticker, d]) => (
          <div key={label} onClick={() => setActive(label)}
            className="bg-gray-900 rounded p-2.5 cursor-pointer border transition-colors"
            style={{ borderColor: active === label ? "#60a5fa" : "#1f2937" }}>
            <div className="text-gray-500 text-xs font-mono mb-1">{label} Crude ({ticker})</div>
            <div className="text-white text-sm font-mono font-bold">{d ? "$" + fmt.price(d.price) : "Loading..."}</div>
            {d && (
              <div className="text-xs font-mono mt-0.5" style={{ color: clr(d.changePct) }}>
                {d.change >= 0 ? "▲" : "▼"} {Math.abs(d.change).toFixed(2)} ({Math.abs(d.changePct).toFixed(2)}%)
              </div>
            )}
            <div className="text-gray-600 text-xs font-mono mt-0.5">per barrel</div>
          </div>
        ))}
      </div>
      <div className="flex items-center justify-between mb-2">
        <span className="text-gray-500 text-xs font-mono">{active} Crude Chart</span>
        <div className="flex border border-gray-800 rounded overflow-hidden">
          {["1W","1M","3M","1Y"].map(t => (
        ...
        {loading ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
              ...
              <YAxis domain={[minP, maxP]} tick={{ fill: "#4b5563", fontSize: 9, fontFamily: "monospace" }} tickLine={false} axisLine={false} tickFormatter={v => "$" + v.toFixed(0)} width={45} />
              ...
              <Area type="monotone" dataKey="price" stroke={lc} strokeWidth={1.5} fill="url(#oilGrad)" dot={false} activeDot={{ r: 3, fill: lc }} />
            </AreaChart>
          </ResponsiveContainer>
        )}
      </div>
